...
```{r, label = "vsa", echo = FALSE, fig.cap = "Validation set approach", eval = FALSE}
...
```{r, label = "LOOC", echo = FALSE, fig.cap = "Leave-one-out cross validation", eval = FALSE}
...
```{r, label = "KFCV", echo = FALSE, fig.cap = "Five fold cross-validation", eval = FALSE}
...
library(PASWR2)
n <- nrow(HSWRESTLER)
train <- sample(n, floor(0.8*n), replace = FALSE)
trainSET <- HSWRESTLER[train,]
testSET <- HSWRESTLER[(-train),]
dim(trainSET)
dim(testSET)
modf <- lm(hwfat ~abs + triceps, data= trainSET)
pred <- predict(modf,newdata= testSET)
MSPE <- mean((testSET$hwfat - pred)^2)
c(MSPE, sqrt(MSPE))
...
lines(1:10, MSPE, col = rgb(j/10,0,0))
points(1:10, MSPE, col = rgb(j/10,0,0), pch = 19)}
...
```{r}
set.seed(1)
glm.modf <- glm(hwfat ~abs + triceps, data= HSWRESTLER)
cv.err <- cv.glm(data = HSWRESTLER, glmfit = glm.modf, K = 8)$delta[1]
cv.err
```
...
for(m in 1:10){
  ...
  lines(1:10, cv, col = rgb(m/10,0,0))
  points(1:10, cv, col = rgb(m/10,0,0), pch = 19)}
```
...
for(j in 1:9){
  cv.err = numeric(10)
    for(i in 1:k){
      ...
      cv.err[i] <- cv.glm(data = Auto, glmfit = glm.fit, K =k)$delta[1]
    }
    lines(1:10, cv.err, col = rgb(j/10,0,0))
    points(1:10, cv.err, col = rgb(j/10,0,0), pch = 19)
  }
...
k <- nrow(HSWRESTLER)
MSPE <- numeric(k)
folds <- sample(x = 1:k, size = nrow(HSWRESTLER), replace = FALSE)  
...
h <- hatvalues(modf)
CVn <- mean(((HSWRESTLER$hwfat - predict(modf))/(1 - h))^2)
CVn
```
...
cv.err <- cv.glm(data = HSWRESTLER, glmfit = glm.fit)$delta[1]
cv.err
```
...
```{r}
plot(1:10, type ="n", xlab = "Degree of Polynomial", ylim = c(16, 26), 
     ylab = "Mean Squared Prediction Error", main = "LOOCV") 

cv.err = numeric(10)

for(j in 1:10){
    glm.fit <- glm(mpg ~ poly(horsepower, j, raw = TRUE), data = Auto)
    cv.err[j] <- cv.glm(data = Auto, glmfit = glm.fit)$delta[1]
}

lines(1:10, cv.err)
points(1:10, cv.err, pch = 19)
```
...
for(j in 1:10){
    glm.fit <- glm(mpg ~ poly(horsepower, j, raw = TRUE), data = Auto)
    h <- hatvalues(glm.fit)
    cv.err[j] <- mean(((Auto$mpg - predict(glm.fit))/(1 - h))^2)
}
